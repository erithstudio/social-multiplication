...
public interface ScoreCardRepository extends
     ...
     */
    @Query("SELECT SUM(s.score) FROM microservices.template.gamification.domain.ScoreCard s WHERE s.userId =:userId GROUP BY s.userId")
     ...
     */
    @Query("SELECT NEW microservices.template.gamification.domain.LeaderBoardRow(s.userId, SUM(s.score)) " +
            "FROM microservices.template.gamification.domain.ScoreCard s " +
